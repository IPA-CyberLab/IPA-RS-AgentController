...
try {
    ...
    New-Item -ItemType Directory -Force -Path (Join-Path $source "disposition-delete-dir") | Out-Null
    ...
    Set-Content -Path (Join-Path $source "force-image-delete.txt") -Value "force-image-delete-original"
    ...
    $readonlyDeleteDir = Join-Path $source "readonly-delete-dir"
    New-Item -ItemType Directory -Force -Path $readonlyDeleteDir | Out-Null
    $readonlyDeleteDirItem = Get-Item $readonlyDeleteDir
    $readonlyDeleteDirItem.Attributes = $readonlyDeleteDirItem.Attributes -bor [IO.FileAttributes]::ReadOnly
...
public static class AgentFsNativeMove {
    ...
    private const uint FILE_DISPOSITION_FORCE_IMAGE_SECTION_CHECK = 0x00000004;
    ...
    public static void DeleteWithForceImageSectionCheck(string path) {
        using (SafeFileHandle handle = CreateFile(
            path,
            DELETE_ACCESS,
            FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
            ...
            FILE_DISPOSITION_INFO_EX info = new FILE_DISPOSITION_INFO_EX();
            info.Flags = FILE_DISPOSITION_DELETE | FILE_DISPOSITION_FORCE_IMAGE_SECTION_CHECK;
            if (!SetFileInformationByHandle(handle, FileDispositionInfoEx, ref info, Marshal.SizeOf(typeof(FILE_DISPOSITION_INFO_EX)))) {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
        }
    ...
    public static void DeleteDirectoryByDispositionInfo(string path) {
        using (SafeFileHandle handle = CreateFile(
            path,
            DELETE_ACCESS,
            FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
    ...
    public static void RenameWithPosixSemantics(string sourcePath, string targetPath) {
        using (SafeFileHandle handle = CreateFile(
            ...
            byte[] targetBytes = System.Text.Encoding.Unicode.GetBytes(targetPath);
            byte[] info = new byte[20 + targetBytes.Length];
            BitConverter.GetBytes(FILE_RENAME_POSIX_SEMANTICS).CopyTo(info, 0);
            BitConverter.GetBytes((long)0).CopyTo(info, 8);
            BitConverter.GetBytes(targetBytes.Length).CopyTo(info, 16);
            Buffer.BlockCopy(targetBytes, 0, info, 20, targetBytes.Length);
    ...
    public static void RenameSuppressPinStateInheritance(string sourcePath, string targetPath) {
        using (SafeFileHandle handle = CreateFile(
            sourcePath,
            DELETE_ACCESS,
            FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
            ...
            byte[] targetBytes = System.Text.Encoding.Unicode.GetBytes(targetPath);
            byte[] info = new byte[20 + targetBytes.Length];
            BitConverter.GetBytes(FILE_RENAME_SUPPRESS_PIN_STATE_INHERITANCE).CopyTo(info, 0);
            BitConverter.GetBytes((long)0).CopyTo(info, 8);
            BitConverter.GetBytes(targetBytes.Length).CopyTo(info, 16);
            Buffer.BlockCopy(targetBytes, 0, info, 20, targetBytes.Length);
    ...
    public static void RenameWithRootDirectory(string sourcePath, string targetDirectory, string targetFileName) {
        ...
        using (SafeFileHandle rootHandle = CreateFile(
            ...
            IO_STATUS_BLOCK iosb;
            int status = NtSetInformationFile(sourceHandle, out iosb, info, (uint)info.Length, NtFileRenameInformation);
            if (status < 0) {
                throw new IOException("NtSetInformationFile rename failed: 0x" + status.ToString("x8"));
            }
        }
    }
...
[AgentFsNativeMove]::DeleteDirectoryOnCloseIgnoreReadonly((Join-Path (Get-Location) 'readonly-delete-dir'))
if (Test-Path readonly-delete-dir) { throw 'FileDispositionInfoEx delete left readonly lower directory visible' }
...
[AgentFsNativeMove]::DeleteWithPosixSemantics((Join-Path (Get-Location) 'posix-delete.txt'))
if (Test-Path posix-delete.txt) { throw 'FileDispositionInfoEx POSIX delete left lower file visible' }
[AgentFsNativeMove]::DeleteWithForceImageSectionCheck((Join-Path (Get-Location) 'force-image-delete.txt'))
if (Test-Path force-image-delete.txt) { throw 'FileDispositionInfoEx force-image delete left lower file visible' }
[AgentFsNativeMove]::DeleteDirectoryByDispositionInfo((Join-Path (Get-Location) 'disposition-delete-dir'))
if (Test-Path disposition-delete-dir) { throw 'FileDispositionInfo delete left lower directory visible' }
[AgentFsNativeMove]::RenameWithRootDirectory(
    (Join-Path (Get-Location) 'rootdir-rename-source.txt'),
    (Get-Location).Path,
    'rootdir-renamed.txt')
if (Test-Path rootdir-rename-source.txt) { throw 'RootDirectory FileRenameInfo left lower source visible' }
if ((Get-Content rootdir-renamed.txt) -ne 'rootdir-rename-original') { throw 'RootDirectory FileRenameInfo target readback failed' }
...
Set-Content suppress-pin-rename-source.txt 'suppress-pin-rename-source-env'
[AgentFsNativeMove]::RenameSuppressPinStateInheritance(
    (Join-Path (Get-Location) 'suppress-pin-rename-source.txt'),
...
if (`$names -notcontains 'rootdir-renamed.txt') { throw 'directory listing lost RootDirectory FileRenameInfo target' }
...
if (`$names -contains 'disposition-delete-dir') { throw 'directory listing showed FileDispositionInfo-deleted lower directory' }
if (`$names -contains 'posix-delete.txt') { throw 'directory listing showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$names -contains 'force-image-delete.txt') { throw 'directory listing showed FileDispositionInfoEx force-image-deleted lower file' }
...
if (`$names -contains 'readonly-delete-dir') { throw 'directory listing showed readonly disposition-deleted lower directory' }
...
if (`$names -contains 'rootdir-rename-source.txt') { throw 'directory listing showed RootDirectory FileRenameInfo source' }
...
if (`$txtNames -notcontains 'rootdir-renamed.txt') { throw 'wildcard listing lost RootDirectory FileRenameInfo target' }
...
if (`$txtNames -contains 'posix-delete.txt') { throw 'wildcard listing showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$txtNames -contains 'force-image-delete.txt') { throw 'wildcard listing showed FileDispositionInfoEx force-image-deleted lower file' }
...
if (`$txtNames -contains 'rootdir-rename-source.txt') { throw 'wildcard listing showed RootDirectory FileRenameInfo source' }
...
if (`$nativeTxtNames -notcontains 'rootdir-renamed.txt') { throw 'native wildcard listing lost RootDirectory FileRenameInfo target' }
...
if (`$nativeTxtNames -contains 'posix-delete.txt') { throw 'native wildcard listing showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$nativeTxtNames -contains 'force-image-delete.txt') { throw 'native wildcard listing showed FileDispositionInfoEx force-image-deleted lower file' }
...
if (`$nativeTxtNames -contains 'rootdir-rename-source.txt') { throw 'native wildcard listing showed RootDirectory FileRenameInfo source' }
...
`$nativeExactRootDirRenameTarget = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'rootdir-renamed.txt', 12, 8, 12)
if (`$nativeExactRootDirRenameTarget -ne 'rootdir-renamed.txt') { throw 'native exact listing lost RootDirectory FileRenameInfo target' }
...
`$nativeExactRootDirRenameSource = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'rootdir-rename-source.txt', 12, 8, 12)
if (`$nativeExactRootDirRenameSource -contains 'rootdir-rename-source.txt') { throw 'native exact listing showed RootDirectory FileRenameInfo source' }
...
`$nativeExactPosixDeleted = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'posix-delete.txt', 12, 8, 12)
if (`$nativeExactPosixDeleted -contains 'posix-delete.txt') { throw 'native exact listing showed FileDispositionInfoEx POSIX-deleted lower file' }
`$nativeExactForceImageDeleted = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'force-image-delete.txt', 12, 8, 12)
if (`$nativeExactForceImageDeleted -contains 'force-image-delete.txt') { throw 'native exact listing showed FileDispositionInfoEx force-image-deleted lower file' }
`$nativeExactDispositionDeletedDir = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'disposition-delete-dir', 12, 8, 12)
if (`$nativeExactDispositionDeletedDir -contains 'disposition-delete-dir') { throw 'native exact listing showed FileDispositionInfo-deleted lower directory' }
`$nativeExactReadonlyDeletedDir = [AgentFsNativeMove]::QueryDirectoryNamesPattern((Get-Location).Path, 'readonly-delete-dir', 12, 8, 12)
if (`$nativeExactReadonlyDeletedDir -contains 'readonly-delete-dir') { throw 'native exact listing showed readonly disposition-deleted lower directory' }
...
if ((Get-ChildItem -Name posix-delete.txt -ErrorAction SilentlyContinue) -contains 'posix-delete.txt') { throw 'exact listing showed FileDispositionInfoEx POSIX-deleted lower file' }
if ((Get-ChildItem -Name force-image-delete.txt -ErrorAction SilentlyContinue) -contains 'force-image-delete.txt') { throw 'exact listing showed FileDispositionInfoEx force-image-deleted lower file' }
if ((Get-ChildItem -Name disposition-delete-dir -ErrorAction SilentlyContinue) -contains 'disposition-delete-dir') { throw 'exact listing showed FileDispositionInfo-deleted lower directory' }
if ((Get-ChildItem -Name readonly-delete-dir -ErrorAction SilentlyContinue) -contains 'readonly-delete-dir') { throw 'exact listing showed readonly disposition-deleted lower directory' }
...
if ((Get-ChildItem -Name rootdir-renamed.txt) -ne 'rootdir-renamed.txt') { throw 'exact listing lost RootDirectory FileRenameInfo target' }
if ((Get-ChildItem -Name rootdir-rename-source.txt -ErrorAction SilentlyContinue) -contains 'rootdir-rename-source.txt') { throw 'exact listing showed RootDirectory FileRenameInfo source' }
...
if (`$singleNames -notcontains 'rootdir-renamed.txt') { throw 'single-entry listing lost RootDirectory FileRenameInfo target' }
...
if (`$singleNames -contains 'disposition-delete-dir') { throw 'single-entry listing showed FileDispositionInfo-deleted lower directory' }
if (`$singleNames -contains 'posix-delete.txt') { throw 'single-entry listing showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$singleNames -contains 'force-image-delete.txt') { throw 'single-entry listing showed FileDispositionInfoEx force-image-deleted lower file' }
if (`$singleNames -contains 'readonly-delete-dir') { throw 'single-entry listing showed readonly disposition-deleted lower directory' }
...
if (`$singleNames -contains 'rootdir-rename-source.txt') { throw 'single-entry listing showed RootDirectory FileRenameInfo source' }
...
if (`$fileIdExtdNames -notcontains 'rootdir-renamed.txt') { throw 'FileIdExtdDirectoryInformation lost RootDirectory FileRenameInfo target' }
...
if (`$fileIdExtdNames -contains 'disposition-delete-dir') { throw 'FileIdExtdDirectoryInformation showed FileDispositionInfo-deleted lower directory' }
if (`$fileIdExtdNames -contains 'posix-delete.txt') { throw 'FileIdExtdDirectoryInformation showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$fileIdExtdNames -contains 'force-image-delete.txt') { throw 'FileIdExtdDirectoryInformation showed FileDispositionInfoEx force-image-deleted lower file' }
if (`$fileIdExtdNames -contains 'readonly-delete-dir') { throw 'FileIdExtdDirectoryInformation showed readonly disposition-deleted lower directory' }
...
if (`$fileIdExtdNames -contains 'rootdir-rename-source.txt') { throw 'FileIdExtdDirectoryInformation showed RootDirectory FileRenameInfo source' }
...
if (`$fileIdExtdBothNames -notcontains 'rootdir-renamed.txt') { throw 'FileIdExtdBothDirectoryInformation lost RootDirectory FileRenameInfo target' }
...
if (`$fileIdExtdBothNames -contains 'disposition-delete-dir') { throw 'FileIdExtdBothDirectoryInformation showed FileDispositionInfo-deleted lower directory' }
if (`$fileIdExtdBothNames -contains 'posix-delete.txt') { throw 'FileIdExtdBothDirectoryInformation showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$fileIdExtdBothNames -contains 'force-image-delete.txt') { throw 'FileIdExtdBothDirectoryInformation showed FileDispositionInfoEx force-image-deleted lower file' }
if (`$fileIdExtdBothNames -contains 'readonly-delete-dir') { throw 'FileIdExtdBothDirectoryInformation showed readonly disposition-deleted lower directory' }
...
if (`$fileIdExtdBothNames -contains 'rootdir-rename-source.txt') { throw 'FileIdExtdBothDirectoryInformation showed RootDirectory FileRenameInfo source' }
...
if (`$fileId64ExtdNames -notcontains 'rootdir-renamed.txt') { throw 'FileId64ExtdDirectoryInformation lost RootDirectory FileRenameInfo target' }
...
if (`$fileId64ExtdNames -contains 'disposition-delete-dir') { throw 'FileId64ExtdDirectoryInformation showed FileDispositionInfo-deleted lower directory' }
if (`$fileId64ExtdNames -contains 'posix-delete.txt') { throw 'FileId64ExtdDirectoryInformation showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$fileId64ExtdNames -contains 'force-image-delete.txt') { throw 'FileId64ExtdDirectoryInformation showed FileDispositionInfoEx force-image-deleted lower file' }
if (`$fileId64ExtdNames -contains 'readonly-delete-dir') { throw 'FileId64ExtdDirectoryInformation showed readonly disposition-deleted lower directory' }
...
if (`$fileId64ExtdNames -contains 'rootdir-rename-source.txt') { throw 'FileId64ExtdDirectoryInformation showed RootDirectory FileRenameInfo source' }
...
if (`$fileId64ExtdBothNames -notcontains 'rootdir-renamed.txt') { throw 'FileId64ExtdBothDirectoryInformation lost RootDirectory FileRenameInfo target' }
...
if (`$fileId64ExtdBothNames -contains 'disposition-delete-dir') { throw 'FileId64ExtdBothDirectoryInformation showed FileDispositionInfo-deleted lower directory' }
if (`$fileId64ExtdBothNames -contains 'posix-delete.txt') { throw 'FileId64ExtdBothDirectoryInformation showed FileDispositionInfoEx POSIX-deleted lower file' }
if (`$fileId64ExtdBothNames -contains 'force-image-delete.txt') { throw 'FileId64ExtdBothDirectoryInformation showed FileDispositionInfoEx force-image-deleted lower file' }
if (`$fileId64ExtdBothNames -contains 'readonly-delete-dir') { throw 'FileId64ExtdBothDirectoryInformation showed readonly disposition-deleted lower directory' }
...
"@
    ...
    if ((Get-Content (Join-Path $source "posix-delete.txt")) -ne "posix-delete-original") {
        throw "host posix-delete.txt was modified"
    }
    if ((Get-Content (Join-Path $source "force-image-delete.txt")) -ne "force-image-delete-original") {
        throw "host force-image-delete.txt was modified"
    }
    if (-not (Test-Path (Join-Path $source "disposition-delete-dir"))) {
        throw "host disposition-delete-dir was removed"
    }
    if ((Get-Content (Join-Path $source "rootdir-rename-source.txt")) -ne "rootdir-rename-original") {
        throw "host rootdir-rename-source.txt was modified"
    }
    ...
    if (Test-Path (Join-Path $source "suppress-pin-rename-source.txt")) {
        throw "host suppress-pin FileRenameInfoEx source was created"
    }
    ...
    if (-not (Test-Path (Join-Path $source "readonly-delete-dir"))) {
        throw "host readonly-delete-dir was removed"
    }
    if (((Get-Item (Join-Path $source "readonly-delete-dir")).Attributes -band [IO.FileAttributes]::ReadOnly) -eq 0) {
        throw "host readonly-delete-dir lost the readonly attribute"
    }
    ...
    if ((Get-Content (Join-Path $upperSource "rootdir-renamed.txt")) -ne "rootdir-rename-original") {
        throw "RootDirectory FileRenameInfo target was not written to upper"
    }
    if (Test-Path (Join-Path $upperSource "rootdir-rename-source.txt")) {
        throw "RootDirectory FileRenameInfo source was copied to upper"
    }
    ...
    if (Test-Path (Join-Path $upperSource "suppress-pin-rename-source.txt")) {
        throw "suppress-pin FileRenameInfoEx source remained in upper"
    }
    ...
    if (Test-Path (Join-Path $upperSource "posix-delete.txt")) {
        throw "FileDispositionInfoEx POSIX-deleted lower file was unexpectedly copied to upper"
    }
    if (Test-Path (Join-Path $upperSource "force-image-delete.txt")) {
        throw "FileDispositionInfoEx force-image-deleted lower file was unexpectedly copied to upper"
    }
    if (Test-Path (Join-Path $upperSource "disposition-delete-dir")) {
        throw "FileDispositionInfo-deleted lower directory was unexpectedly copied to upper"
    }
    ...
    if (Test-Path (Join-Path $upperSource "readonly-delete-dir")) {
        throw "readonly disposition-deleted lower directory was unexpectedly left in upper"
    }
    ...
    if (-not (Test-Path (Join-Path $whiteoutSource "posix-delete.txt"))) {
        throw "FileDispositionInfoEx POSIX delete whiteout was not created"
    }
    if (-not (Test-Path (Join-Path $whiteoutSource "force-image-delete.txt"))) {
        throw "FileDispositionInfoEx force-image delete whiteout was not created"
    }
    if (-not (Test-Path (Join-Path $whiteoutSource "disposition-delete-dir"))) {
        throw "FileDispositionInfo directory delete whiteout was not created"
    }
    if (-not (Test-Path (Join-Path $whiteoutSource "rootdir-rename-source.txt"))) {
        throw "RootDirectory FileRenameInfo source whiteout was not created"
    }
    ...
    if (-not (Test-Path (Join-Path $whiteoutSource "readonly-delete-dir"))) {
        throw "readonly disposition-delete directory whiteout was not created"
    }
    ...
    if (Test-Path (Join-Path $whiteoutSource "suppress-pin-rename-source.txt")) {
        throw "suppress-pin FileRenameInfoEx upper-only rename created a source whiteout"
    }
